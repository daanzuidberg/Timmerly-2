...
export function ContentPage({ eyebrow, title, intro, children }: { eyebrow: string; title: string; intro: string; children: ReactNode }) {
  return (
    <>
      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-4 px-6 py-12 [&_section]:rounded-card [&_section]:border [&_section]:border-line-soft [&_section]:bg-white [&_section]:p-6 [&_section]:shadow-card [&_h2]:text-lg [&_h2]:font-bold [&_h2]:mb-2 [&_p]:text-[15px] [&_p]:leading-relaxed [&_p]:text-navy-500 [&_li]:text-[15px] [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:flex [&_ul]:flex-col [&_ul]:gap-1">{children}</div>
    </>
  );
}
