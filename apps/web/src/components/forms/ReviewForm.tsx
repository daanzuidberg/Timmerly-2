    ...
    <form action={action} className="flex flex-col gap-4">
      ...
      {categories.map((c) => (
        <fieldset key={c} className="flex flex-wrap items-center justify-between gap-2 border-b border-line pb-3">
          ...
          <div className="flex gap-1">{[1, 2, 3, 4, 5].map((n) => <label key={n} className="cursor-pointer"><input type="radio" name={`score.${c}`} value={n} className="peer sr-only" required /><span className="inline-flex h-9 w-9 items-center justify-center rounded-control border border-line text-sm font-bold peer-checked:border-orange peer-checked:bg-orange peer-checked:text-white">{n}</span></label>)}</div>
        </fieldset>
      ))}
