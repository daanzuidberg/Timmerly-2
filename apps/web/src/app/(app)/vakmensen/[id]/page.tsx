    ...
    <AppShell user={user} title={name} subtitle={`${TRADE_LABELS[p.trade as Trade]} · ${p.city} · ${p.yearsExperience} jaar ervaring`}>
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-5">
          <Card>
            <div className="flex items-center gap-4"><Avatar name={revealed ? name : TRADE_LABELS[p.trade as Trade]} size={56} /><div><div className="flex flex-wrap gap-1.5">{badges.filter((b) => b[1]).map(([l]) => <Pill key={String(l)} tone="ok">✓ {l}</Pill>)}</div><div className="mt-2 flex flex-wrap gap-1">{p.specialisms.map((s) => <span key={s} className="rounded-[3px] bg-ground px-2 py-0.5 text-xs font-bold">{SPECIALISM_LABELS[s as Specialism] ?? s}</span>)}</div></div></div>
